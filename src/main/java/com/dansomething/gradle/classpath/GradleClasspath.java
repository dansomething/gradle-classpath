...
class GradleClasspath {
  static String getClasspath(
    ...
    if (gradleHome != null) {
      ...
      if (!gradleHomeFile.exists()) {
        throw new ClasspathException(String.format(
          "The specified gradle home directory doesn't exist? '%s'", gradleHome
        ));
      }
    ...
    if (!projectPathFile.exists()) {
      throw new ClasspathException(String.format(
        "The specified project directory doesn't exist? '%s'", projectPath
      ));
    }
    ...
    if (!buildGradlePath.toFile().exists()) {
      throw new ClasspathException(String.format(
        "The specified project directory doesn't contain a build.gradle file? '%s'", projectPath
      ));
    }
